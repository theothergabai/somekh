...
export async function loadBonusSignalsData() {
  const url = `./src/data/bonus.json?v=${Date.now()}`;
  const res = await fetch(url, { cache: 'no-store' });
  if (!res.ok) throw new Error(`Failed to load bonus.json: ${res.status}`);
  const data = await res.json();
  if (!Array.isArray(data)) throw new Error('bonus.json must be an array');
  return data.map((s) => ({
    id: s.id,
    name: s.name,
    symbol: s.symbol ?? (Array.isArray(s.symbols) ? s.symbols.join('') : undefined),
